...
const UpdateJob = () => {
    ...
    const { loading, data } = useSelector(state => state.job);


    const formData = new FormData();
    formData.append('logo', logo);
    formData.append("id",'652f76bf6840b4521af16857')
    ...
    useEffect(() => {
        dispatch({ type: ADMIN.FETCH_JOB_REQUEST });
        axios.post(URL + '/admin/api/get-job-details', {
            id: '652f76bf6840b4521af16857'
        }).then(res => {
            ...
            dispatch({ type: ADMIN.FETCH_JOB_SUCCESS, payload: res.data })
        }).catch(err => {
            dispatch({ type: ADMIN.FETCH_JOB_SUCCESS, payload: err.response.data.message })
        })
    }, [])
    ...
    const HandleSubmit = async (e) => {
        ...
        if (!err.length > 0) {
            ...
            try {
                ...
                axios.post(URL + '/admin/api/update-job',
                    formData
                ).then(res => {
                    console.log(res);
                    if (res.status === 200) {
                        ...
                        <form id="Form">
                            ...
                            <div className="Name">
                                <li><label>Company name:</label>
                                    <input type="text" name='company_name' value={formValues.company_name} onChange={(e) => HandleChange(e, 'company_name')} placeholder="Enter name" />
                                    <Alert label={'company_name'} errors={errors} />
                                </li>
                                <li><label>Designation:</label>
                                    <input type="text" name='designation' value={formValues.designation} onChange={(e) => HandleChange(e, 'designation')} placeholder="Enter designation" />
                                    <Alert label={'designation'} errors={errors} />
                                </li>
                            </div>
                            <li>
                                <label>Description:</label>
                                <textarea type="text" className='textarea' value={formValues.description} name='description' onChange={(e) => HandleChange(e, 'description')} placeholder="Enter vehicle description" />
                                <Alert label={'description'} errors={errors} />
                            </li>
                            <li><label>Qualificatoins:</label>
                                <textarea type="text" name='qualifications' value={formValues.qualifications} className='textarea' onChange={(e) => HandleChange(e, 'qualifications')} placeholder="Enter qualifications" />
                                <Alert label={'qualifications'} errors={errors} />
                            </li>
                            <div className="Name">
                                <li><label>Vacancy:</label>
                                    <input type="number" name='vacancy' value={formValues.vacancy} onChange={(e) => HandleChange(e, 'vacancy')} placeholder="Enter vacancy" />
                                    <Alert label={'vacancy'} errors={errors} />
                                </li>
                                <li><label>Date line:</label>
                                    <input type="date" name='dateline' value={formValues.dateline} onChange={(e) => HandleChange(e, 'dateline')} placeholder="Enter dateline" />
                                    <Alert label={'dateline'} errors={errors} />
                                </li>
                            </div>
                            <li><label>Location:</label>
                                <input type="text" name='location' value={formValues.location} onChange={(e) => HandleChange(e, 'location')} placeholder="Enter location" />
